...
const weekStatus = getWeekStatus(week.entries, 'HR');
				...
				<header className="bg-white px-4 md:px-8 py-4 flex flex-wrap items-center justify-between shadow-sm z-10 border-b border-[#E3E8EF]">
					<div className="flex items-center gap-6">
						...
						<div className="flex items-center gap-3 relative" id="profile-dropdown-container">
							...
							className="w-10 h-10 rounded-full border-2 border-brand-yellow overflow-hidden cursor-pointer hover:scale-105 active:scale-95 transition-all shadow-lg flex items-center justify-center bg-brand-yellow/20 p-0"
							title="My Profile"
						>
							{user.photoPath ? (
							...
							{isProfileDropdownOpen && (
								<div className="absolute right-0 top-full mt-2 w-64 bg-white rounded-2xl shadow-2xl border border-brand-blue/5 py-2 z-[100] animate-in fade-in zoom-in duration-200 origin-top-right">
									...
									<div className="h-px bg-brand-blue/5 mx-2 my-2"></div>
									<button
										onClick={() => {
							...
							<div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4 md:gap-6">
								{loading ? (
									...
									[
										...
										{ label: "Maternity", value: (leaveBalance.maternityLeavesRemaining || 0).toFixed(0).padStart(2, '0'), color: "text-pink-500" },
										{ label: "Paternity", value: (leaveBalance.paternityLeavesRemaining || 0).toFixed(0).padStart(2, '0'), color: "text-indigo-500" },
										{ label: "Bereavement", value: (leaveBalance.bereavementLeavesRemaining || 0).toFixed(0).padStart(2, '0'), color: "text-slate-500" },
									].map((stat, index) => (
															...
															<td colSpan={5} className="p-0 bg-bg-slate border-b border-brand-blue/5">
																<div className="px-6 py-4">
																	<div className="flex items-center justify-between mb-3">
																		...
																		<span className={`px-2 py-0.5 rounded-full text-[8px] font-black uppercase tracking-widest ${week.status === 'APPROVED' ? 'bg-emerald-100 text-emerald-600' : week.status === 'REJECTED' ? 'bg-red-100 text-red-600' : 'bg-brand-yellow/10 text-brand-yellow'}`}>{week.status}</span>
																	</div>
											...
											{loading ? (
												...
												recentLeaves.map((leave) => (
													<tr key={leave.id} className="hover:bg-bg-slate transition-colors font-medium">
														...
														<td className="p-4 px-6 text-brand-text/70 text-xs text-center whitespace-nowrap">
															{formatDate(leave.startDate)} → {formatDate(leave.endDate)}
														</td>
														<td className="p-4 px-6 text-center">
															<span className="bg-brand-blue/5 text-brand-text px-2 py-0.5 rounded text-[10px] font-black">
														...
														<td className="p-4 px-6 text-right">
															{leave.approvedBy ? (
																<div className="space-y-0.5">
																	...
																	{leave.reviewedAt && (
																		<p className="text-[10px] text-brand-text/40 font-bold">{formatDateTime(leave.reviewedAt)}</p>
																	)}
																</div>
															) : (
												...
												<tr>
													<td colSpan="6" className="p-8 text-center text-brand-text/30 font-bold uppercase tracking-widest text-[10px]">No leave history found</td>
												</tr>
											)}
										</tbody>
									</table>
