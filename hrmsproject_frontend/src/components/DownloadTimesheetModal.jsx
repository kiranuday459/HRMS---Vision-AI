...
export default function DownloadTimesheetModal({ isOpen, onClose, employees: rawEmployees }) {
    ...
    const downloadExcel = async (dateSequence, selectedIds, dateFilteredEntries, allLeaves, allHolidays) => {
        ...
        const sD = parseLocalDate(fromDate);
        const eD = parseLocalDate(toDate);

        const diffTime = Math.abs(eD - sD);
        const daysDiff = Math.ceil((diffTime + 1) / (1000 * 60 * 60 * 24));
        const isMonthlyMode = daysDiff > 30;
        ...
        const headerFill = { type: "pattern", pattern: "solid", fgColor: { argb: "FFD9D2E9" } };
        const titleFill = { type: "pattern", pattern: "solid", fgColor: { argb: "FFD9D9D9" } };

        // Group dates for worksheets
        const groups = [];

        if (!isMonthlyMode) {
            groups.push({
                sheetName: "Weekly Timesheet",
                reportTypeTitle: "WEEKLY TIMESHEET",
                dates: dateSequence
            });
        } else {
            const monthMap = {};
            ...
            Object.keys(monthMap).sort().forEach((mKey) => {
                const sampleDate = parseLocalDate(monthMap[mKey][0]);
                const monthLong = sampleDate.toLocaleDateString("en-US", { month: "long" });
                const yearFull = sampleDate.getFullYear();
                const sheetName = `${monthLong} ${yearFull}`;
                const reportTypeTitle = `MONTHLY TIMESHEET — ${monthLong.toUpperCase()} ${yearFull}`;
                groups.push({
                    sheetName,
                    reportTypeTitle,
                    dates: monthMap[mKey]
        ...
        groups.forEach((group) => {
            ...
            const datesForSheet = group.dates;
            const colCount = 6 + datesForSheet.length + summaryHeaders.length;

            const sheetStartStr = formatDMY(datesForSheet[0]);
            const sheetEndStr = formatDMY(datesForSheet[datesForSheet.length - 1]);

            // ROW 1: Main Title
            worksheet.mergeCells(1, 1, 1, colCount);
            const row1 = worksheet.getRow(1);
            row1.getCell(1).value = `VISIONAI HRMS SYSTEM — ${group.reportTypeTitle} — (Cycle: ${startStr} → ${endStr})`;
            row1.height = 30;
            row1.getCell(1).fill = titleFill;
            row1.getCell(1).font = { color: { argb: "FF000000" }, bold: true, size: 13 };
            row1.getCell(1).alignment = { horizontal: "center", vertical: "middle" };

            // ROW 2: Sub-header / Legend
            ...
            worksheet.mergeCells(3, 7, 3, 6 + datesForSheet.length); // Day Wise Attendance
            row3.getCell(7).value = `DAY WISE ATTENDANCE (${sheetStartStr} → ${sheetEndStr})`;

            worksheet.mergeCells(3, 7 + datesForSheet.length, 3, colCount); // Summary
            row3.getCell(7 + datesForSheet.length).value = isMonthlyMode ? "MONTHLY SUMMARY" : "PERIOD SUMMARY";

            for (let c = 1; c <= colCount; c++) {
                const cell = row3.getCell(c);
                cell.fill = headerFill;
                cell.font = { color: { argb: "FF000000" }, bold: true, size: 10 };
            ...
            baseHeaders.forEach((h, i) => {
                ...
                const leftAlign = (colIdx === 3 || colIdx === 4);
                [row4, row5].forEach(r => {
                    const c = r.getCell(colIdx);
                    c.fill = headerFill;
                    c.font = { color: { argb: "FF000000" }, bold: true, size: 9.5 };
                    c.alignment = { horizontal: leftAlign ? "left" : "center", vertical: "middle" };
                    c.border = thinBorder;
                });
            });

            datesForSheet.forEach((ds, i) => {
                const colIdx = 7 + i;
            ...
            summaryHeaders.forEach((h, i) => {
                const colIdx = 7 + datesForSheet.length + i;
                worksheet.mergeCells(4, colIdx, 5, colIdx);
                row4.getCell(colIdx).value = h;
                [row4, row5].forEach(r => {
                    const c = r.getCell(colIdx);
            ...
            // Freeze Panes — freeze Rows 1–5 so headers remain fixed while scrolling
            worksheet.views = [
            ...
            selectedIds.forEach((empId, empIdx) => {
                ...
                const firstSheetDate = parseLocalDate(datesForSheet[0]);
                ...
                r.getCell(5).value = firstSheetDate.toLocaleDateString("en-US", { month: "short" });
                r.getCell(6).value = firstSheetDate.getFullYear();

                for (let i = 1; i <= 6; i++) {
                    const c = r.getCell(i);
                    c.alignment = { vertical: 'middle', horizontal: (i === 3 || i === 4) ? 'left' : 'center' };
                    c.font = { size: 9.5, bold: i <= 3 };
                    c.border = thinBorder;
                    c.fill = { type: "pattern", pattern: "solid", fgColor: { argb: i <= 4 ? "FFDDD8E8" : "FFC4BCDA" } };
                }
                ...
                const totalWorkingDays = datesForSheet.length - weekOffs - holidays;
                const attPerc = totalWorkingDays > 0 ? (presDays / totalWorkingDays) * 100 : 0;

                const sumStart = 7 + datesForSheet.length;
            ...
            worksheet.getColumn(2).width = 14; // Emp ID
            worksheet.getColumn(3).width = 26; // Name
            worksheet.getColumn(4).width = 20; // Department
            worksheet.getColumn(5).width = 10; // Month
            worksheet.getColumn(6).width = 8;  // Year

            for (let i = 0; i < datesForSheet.length; i++) {
                worksheet.getColumn(7 + i).width = 5;
            }
